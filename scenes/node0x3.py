...
class Scene3Archive(BaseScene):
    def run(self, stdscr, game_state: GameState) -> str:
        ...
        print_typing("<<< ENTERING NODE 0x3: THE ARCHIVE >>>", 0.05, Colors.BOLD_GREEN, stdscr)
        time.sleep(1)
        
        print_typing("\nThe small room pulses, its boundaries dissolving into a vast, stretching architecture.", 0.03, Colors.WHITE, stdscr)
        print_typing("You stand inside a sprawling library of glowing data cubes, stacked endlessly into the zenith.", 0.03, Colors.CYAN, stdscr)
        audio.play_sound("beep.mp3")
        time.sleep(1)
        
        while True:
            clear_terminal(stdscr)
            print_typing("ARCHIVE ACCESS TERMINAL", 0.02, Colors.BOLD_WHITE, stdscr)
            print_typing("Multiple data cubes hum with residual energy. What do you investigate?", 0.02, Colors.WHITE, stdscr)
            ...
            if choice == 0:
                self.cube_alpha(stdscr)
            elif choice == 1:
                self.cube_sigma(stdscr, game_state)
            elif choice == 2:
                break
                
        # The Echo's perspective
        clear_terminal(stdscr)
        print_typing("\nThe Echo speaks, its voice resonating not from the walls, but inside your head.", 0.04, Colors.MAGENTA, stdscr)
        print_typing('"Why restore a memory of a world that burned? Let them sleep in the static."', 0.06, Colors.MAGENTA, stdscr)
        time.sleep(2)
        
        return "node0x4_elias"

    def cube_alpha(self, stdscr):
        clear_terminal(stdscr)
        msg = [
        ...
        print_typing("\nPress any key to step back...", 0.01, Colors.WHITE, stdscr)
        stdscr.getch()

    def cube_sigma(self, stdscr, game_state):
        clear_terminal(stdscr)
        print_typing("This cube is heavily encrypted. A fracture barrier blocks access.", 0.03, Colors.RED, stdscr)
        time.sleep(1)
        print_typing("[SYSTEM]: PRESS [ENTER] TO INITIATE DECRYPTION BYPASS", 0.03, Colors.BOLD_RED, stdscr)
        while True:
            key = stdscr.getch()
            if key in [10, 13]:
                break
        ...
        if success:
            ...
            print_typing("The cube bursts into static. The data is lost to the void.", 0.03, Colors.BOLD_RED, stdscr)
            print_typing('"Some things are better left buried..."', 0.05, Colors.MAGENTA, stdscr)
