    ...
    def display(self, stdscr, duration=None, getch_func=None) -> int:
        ...
        while True:
            ...
            if key == curses.KEY_UP:
                ...
                # Redraw only the selected choice in green, no arrow prefix
                selected_text = self.choices[self.selected_index]
                stdscr.addstr(
                    menu_start_y,
                    0,
                    selected_text,
                    curses_colors.ansi_1m32,  # bold green, same as hover color
                )
                stdscr.refresh()
                time.sleep(0.4)  # brief hold so the player sees their selection

                # Move cursor below the confirmed choice line
                final_y = min(max_h - 1, menu_start_y + 1)
...
class TimedPuzzle:
    def __init__(
        ...
        bonus_time = (
            game_state.stability // 4 if game_state else 0
        )  # +2s at stability 8
    ...
    def display(self, stdscr, getch_func=None) -> bool:
        ...
        if not self.auto_start:
            ...
            while True:
                ...
                MessageBox.draw_box(
                    stdscr, start_y, start_x, box_width, box_height, Colors.BOLD_MAGENTA
                )
        ...
        while True:
            ...
            print_colored(
                timer_str, timer_color, stdscr=stdscr, y=start_y + 5, x=t_x, end=""
            )
        ...
        if not success:
            ...
            while time.time() - fail_start < 1.0:
                ...
                for i, line in enumerate(msg_lines):
                    glitch_line = "".join(
                        random.choice("@#░▒▓$!%?&") if random.random() < 0.2 else c
                        for c in line
                    )
                    ...
                    print_colored(
                        glitch_line,
                        random.choice([Colors.BOLD_RED, Colors.BOLD_WHITE]),
                        stdscr=stdscr,
                        y=f_y,
                        x=f_x,
                        end="",
        ...
        MessageBox.draw_box(
            stdscr, start_y, start_x, box_width, box_height, current_border_color
        )
    ...
    def draw_box(stdscr, y, x, width, height, color, glitch_prob: float = 0.0):
        ...
        for i in range(height):
            ...
            for idx, ch in enumerate(text):
                ...
                print_colored(
                    char_to_draw, draw_color, stdscr=stdscr, y=y + i, x=x + idx, end=""
                )
    ...
    def display(self, stdscr, duration: float = 0, getch_func=None) -> Union[None, int]:
        ...
        def get_line_len(line):
            ...
            if isinstance(line, list):
                return sum(
                    len(segment[0] if isinstance(segment, tuple) else segment)
                    for segment in line
                )
            return 0
        ...
        while True:
            # Draw Border using centralized logic
            self.draw_box(
                stdscr, start_y, start_x, box_width, box_height, self.border_color
            )
            ...
            for line in self.message:
                ...
                if isinstance(line, str):
                    ...
                    text, color = line
                    print_colored(
                        text, color, stdscr=stdscr, y=current_line_y, x=m_x, end=""
                    )
            ...
            if self.choices:
                ...
                for idx, choice in enumerate(self.choices):
                    ...
                    print_colored(
                        c_str, c_color, stdscr=stdscr, y=current_line_y, x=c_x, end=""
                    )
                    current_line_y += 1
